...
func (r *repository) SaveWeatherDaily(w *WeatherDaily) error {
	query := `
		INSERT INTO weather_daily (id, date, lat, lng, cloud_cover, temperature, shortwave_radiation_mj, raw_json, created_at)
	...
	`
	_, err := r.db.Exec(query, w.ID, normalizeDate(w.Date), w.Lat, w.Lng, w.CloudCover, w.Temperature, w.ShortwaveRadiationMJ, w.RawJSON, w.CreatedAt)
	if err != nil {
...
func (r *repository) GetWeatherByDateAndLocation(date time.Time, lat, lng float64) (*WeatherDaily, error) {
	query := `
		SELECT id, date, lat, lng, cloud_cover, temperature, COALESCE(shortwave_radiation_mj, 0), raw_json, created_at
		FROM weather_daily
	...
	w := &WeatherDaily{}
	if err := row.Scan(&w.ID, &w.Date, &w.Lat, &w.Lng, &w.CloudCover, &w.Temperature, &w.ShortwaveRadiationMJ, &w.RawJSON, &w.CreatedAt); err != nil {
		return nil, err
	}
	return w, nil
}
